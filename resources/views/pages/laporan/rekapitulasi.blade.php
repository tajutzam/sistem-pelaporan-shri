            ...
            <div class="flex gap-4 mt-4 justify-end">
                ...
                @if (auth()->user()->hak_akses == 'kepala')
                    <form method="post" action="{{ route('laporan.verifikasi') }}">
                        ...
                        {{-- Logika perubahan warna: Jika ada $approved pakai green, jika tidak pakai gray --}}
                        <button type="submit" @if ($approved) disabled @endif
                            class="{{ $approved ? 'bg-green-600' : 'bg-gray-600 hover:bg-gray-700' }} text-white px-5 py-2.5 rounded-lg transition font-medium inline-flex items-center gap-2">

                            <i class="fas {{ $approved ? 'fa-check-double' : 'fa-check' }}"></i>

                            <span>
                                {{ $approved ? 'TERVERIFIKASI' : 'VERIFIKASI' }}
                            </span>
                        </button>
                    </form>
                @endif
