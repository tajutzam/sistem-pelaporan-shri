            ...
            ->select([
                    ...
                    WHEN s.tanggal_masuk <= "' . $endDateString . '" THEN
                        GREATEST(
                            DATEDIFF(
                                ...
                                GREATEST(s.tanggal_masuk, "' . $startDateString . '")
                            ) + 1,
                            0
                        )
                    ELSE 0
                ...
                /*                \DB::raw('COALESCE(SUM(
                    ...
                    WHEN s.status = "keluar" AND sk.tanggal_keluar IS NOT NULL 
                        THEN DATEDIFF(LEAST(sk.tanggal_keluar, "' . $endDateOnly . '"), GREATEST(s.tanggal_masuk, "' . $startDateString . '")) 
                    WHEN s.status = "pindah" AND sp.tanggal_pindah IS NOT NULL 
                        THEN DATEDIFF(LEAST(sp.tanggal_pindah, "' . $endDateOnly . '"), GREATEST(s.tanggal_masuk, "' . $startDateString . '")) 
                    WHEN s.status = "masuk" 
...
*/
                ...
                CASE
                    ...
                    WHEN s.status = "pindah" AND sp.tanggal_pindah IS NOT NULL AND sp.tanggal_pindah BETWEEN "' . $startDateString . '" AND "' . $endDateString . '"
