...
class LaporanController extends Controller
{
    ...
    public function rekapitulasi(Request $request)
    {
        ...
        $approved = $this->checkVerifikasi('rekapitulasi');
    ...
    private function checkVerifikasi($jenisLaporan)
    {
        $today = Carbon::today();

        return ApprovedDay::whereDate('created_at', $today)
    ...
    public function exportRekapitulasi(Request $request)
    {
        ...
        if (!$approved) {
            return back()->withErrors('Silahkan menunggu kepala rekam medis untuk memverifikasi');
        }
    ...
    public function tenDiagnosaPenyakit(Request $request)
    {
        ...
        $approved = $this->checkVerifikasi('penyakit');

        return view('pages.laporan.10_besar_penyakit', compact('diagnosas', 'ruangans', 'approved'));
    }
    ...
    public function getLaporanBOR(Request $request)
    {
        ...
        $approved = $this->checkVerifikasi('indikator');

        return view('pages.laporan.indikator_pelayanan', compact('data', 'year', 'month', 'approved'));
    }
    ...
    private function getLaporanIndikator($startDate, $endDate, $ruanganId = null, $month = null)
    {
        ...
        $results = $ruangans->map(function ($ruangan) use ($dtStart, $dtEnd, $jumlahPeriode, $startDate) {
            ...
            $stats = \DB::table('shris as s')
                ...
                ->select([
                    \DB::raw('COALESCE(SUM(
                    CASE
                        WHEN s.tanggal_masuk <= "' . $endDateString . '" THEN
                            GREATEST(
                                DATEDIFF(
                                    LEAST(
                                        COALESCE(
                                            DATE_SUB(sk.tanggal_keluar, INTERVAL 1 DAY),
                                            DATE_SUB(sp.tanggal_pindah, INTERVAL 1 DAY),
                                            "' . $endDateString . '"
                                        ),
                                        "' . $endDateString . '"
                                    ),
                                    GREATEST(s.tanggal_masuk, "' . $dtStart . '")
                                ) + 1,
                                0
                            )
                        ELSE 0
                    ...
                    CASE
                        WHEN s.status = "keluar" AND sk.tanggal_keluar IS NOT NULL 
                            THEN DATEDIFF(sk.tanggal_keluar, s.tanggal_masuk)                         WHEN s.status = "pindah" AND sp.tanggal_pindah IS NOT NULL 
                            THEN DATEDIFF(sp.tanggal_pindah, s.tanggal_masuk) 
                        WHEN s.status = "masuk" 
...
*/
                    \DB::raw('COALESCE(SUM(
                    CASE
                        WHEN s.status = "keluar" AND sk.tanggal_keluar IS NOT NULL AND sk.tanggal_keluar BETWEEN "' . $dtStart . '" AND "' . $endDateString . '"
                            THEN CAST(REPLACE(sk.lama_dirawat, " hari", "") AS UNSIGNED)
                        WHEN s.status = "pindah" AND sp.tanggal_pindah IS NOT NULL AND sp.tanggal_pindah BETWEEN "' . $dtStart . '" AND "' . $endDateString . '"
                            THEN CAST(REPLACE(sp.lama_dirawat, " hari", "") AS UNSIGNED)
                        ELSE 0
                    END
                ), 0) as total_ld'),

                    \DB::raw('COUNT(CASE WHEN s.status = "keluar" AND sk.tanggal_keluar BETWEEN "' . $dtStart . '" AND "' . $endDateString . '" THEN 1 END) as total_keluar'),
                    \DB::raw('COUNT(CASE WHEN s.status = "keluar" AND sk.tanggal_keluar BETWEEN "' . $dtStart . '" AND "' . $endDateString . '" AND sk.cara_keluar NOT LIKE "Mati%" THEN 1 END) as keluar_hidup'),
                    \DB::raw('COUNT(CASE WHEN s.status = "keluar" AND sk.tanggal_keluar BETWEEN "' . $dtStart . '" AND "' . $endDateString . '" AND sk.cara_keluar = "Mati ≥ 48 Jam" THEN 1 END) as mati_lebih_48'),
                    \DB::raw('COUNT(CASE WHEN s.status = "keluar" AND sk.tanggal_keluar BETWEEN "' . $dtStart . '" AND "' . $endDateString . '" AND sk.cara_keluar = "Mati ≤ 48 Jam" THEN 1 END) as mati_kurang_48'),
                ])->first();
