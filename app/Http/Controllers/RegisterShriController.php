...
class RegisterShriController extends Controller
{
    ...
    public function masukUpdate(Request $request, $id)
    {
        $validated = $request->validate([
            ...
            'ruang_perawatan' => 'required|exists:ruangans,id', // Ini ID Ruangan tujuan
            'jenis_penjaminan' => 'required|exists:penjaminans,id',
            'dpjp' => 'required|exists:dpjps,id',
        ]);

        $shri = Shri::findOrFail($id);
        $ruanganTujuan = Ruangan::withoutGlobalScopes()->find($validated['ruang_perawatan']);
        ...
        // --- VALIDASI KAPASITAS ---
        // Cek apakah user pindah ruangan (ruangan lama != ruangan baru)
        // Jika ganti ruangan, cek apakah ruangan tujuan masih tersedia
        if ($shri->ruang_perawatan_id != $ruanganTujuan->id) {
            $tersedia = $this->getTempatTidurTersedia($ruanganTujuan->id);

            if ($tersedia <= 0) {
                return back()->withErrors([
                    'ruang_perawatan' => 'Gagal! Ruangan ' . $ruanganTujuan->nama_ruangan . ' sudah penuh.'
                ])->withInput();
            }
        }
        // --------------------------
        ...
        $shri->update([
            'tanggal_masuk' => $validated['tanggal_masuk'],
            'asal_pasien' => $validated['asal_pasien'],
            'kelas_perawatan_id' => $validated['kelas_perawatan'], // Gunakan ID kelas dari form
            'ruang_perawatan_id' => $ruanganTujuan->id,           // Pastikan nama kolom konsisten (id)
            'ruang_perawatan' => $ruanganTujuan->nama_ruangan,    // Nama ruangan (string)
            'jenis_penjaminan_id' => $validated['jenis_penjaminan'],
            'dpjp_id' => $validated['dpjp'],
        ]);
    ...
    public function pindahStore(Request $request)
    {
        ...
        // 1. Cari ruangan tujuan
        $ruangan = Ruangan::withoutGlobalScopes()
            ->where('nama_ruangan', $validated['nama_ruangan_tujuan'])
            ->first();


        // 2. VALIDASI: Cek apakah tempat tidur masih tersedia di ruangan tujuan
        // Menggunakan fungsi getTempatTidurTersedia yang sudah kita perbaiki tadi
        $tersedia = $this->getTempatTidurTersedia($ruangan->id);



        if ($tersedia <= 0) {
            return redirect()->back()
                ->withErrors(['nama_ruangan_tujuan' => 'Gagal! Ruangan ' . $ruangan->nama_ruangan . ' sudah penuh (tidak ada tempat tidur tersedia).'])
                ->withInput();
        }
        ...
        if ($created) {
            ...
            return redirect()->route('register-shri.pindah.view')->with('success', 'Berhasil memindahkan pasien ke ' . $ruangan->nama_ruangan);
        }
    ...
    private function getTempatTidurTersedia($ruanganId = null)
    {
        ...
        $terisi = Shri::withoutGlobalScopes()
            ->where('status', '!=', 'keluar')
            ->whereDoesntHave('pindah') // Jika ada data di tabel pindah, berarti dia sudah keluar dari bed ini
            ->when($ruanganId, function ($q) use ($ruanganId) {
                $q->where('kelas_perawatan_id', $ruanganId);
            })
            ->count();
