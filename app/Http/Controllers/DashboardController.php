...
class DashboardController extends Controller
{
    ...
    {
        return Shri::whereBetween('tanggal_masuk', [
    ...
    {
        return ShriKeluar::whereBetween('tanggal_keluar', [
            Carbon::today()->startOfDay(),
            Carbon::today()->endOfDay()
        ])->count();
    }
            ...
            ->select([
                    ...
                    WHEN s.tanggal_masuk <= "' . $endDateString . '" THEN
                        GREATEST(
                            DATEDIFF(
                                ...
                                GREATEST(s.tanggal_masuk, "' . $startDate . '")
                            ) + 1,
                            0
                        )
                    ELSE 0
                ...
                CASE
                    WHEN s.status = "keluar" AND sk.tanggal_keluar IS NOT NULL 
                        THEN DATEDIFF(LEAST(sk.tanggal_keluar, "' . $endDateString . '"), GREATEST(s.tanggal_masuk, "' . $startDate . '")) 
                    WHEN s.status = "pindah" AND sp.tanggal_pindah IS NOT NULL 
                        THEN DATEDIFF(LEAST(sp.tanggal_pindah, "' . $endDateString . '"), GREATEST(s.tanggal_masuk, "' . $startDate . '")) 
                    WHEN s.status = "masuk" 
...
*/
                ...
                CASE
                    ...
                    WHEN s.status = "pindah" AND sp.tanggal_pindah IS NOT NULL AND sp.tanggal_pindah BETWEEN "' . $startDate . '" AND "' . $endDate . '"
